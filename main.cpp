...
namespace
{
    ...
    void send_wol(const std::string& hardware_addr, unsigned port, unsigned long bcast)
    {
        ...
        socket_handle packet{static_cast<int>(socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP))};
        ...
        // Set socket options.
        const int optval{1};
        if (setsockopt(packet.get(), SOL_SOCKET, SO_BROADCAST, reinterpret_cast<const char *>(&optval), sizeof(optval)) < 0) {
